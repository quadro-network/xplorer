...
<template>
    <div>
        ...
        <div class="text-center mb-4">
            ...
            <label @click="showFunds()" for="modal-contract-funds" class="btn  #sav1 btn-xs text-xs mr-2">{{
                $t('cosmwasm.btn_funds') }}</label>
            <label class="btn  #sav1 btn-xs text-xs mr-2" for="modal-contract-states" @click="showState()">
                {{ $t('cosmwasm.btn_states') }}
            </label>
            <label for="modal-contract-query" class="btn  #sav1 btn-xs text-xs mr-2" @click="showQuery()">
                {{ $t('cosmwasm.btn_query') }}
            </label>
            <label for="wasm_execute_contract" class="btn  #sav1 btn-xs text-xs mr-2"
                @click="dialog.open('wasm_execute_contract', { contract: contractAddress })">
                {{ $t('cosmwasm.btn_execute') }}
            </label>

            <label for="wasm_migrate_contract" class="btn  #sav1 btn-xs text-xs mr-2"
                @click="dialog.open('wasm_migrate_contract', { contract: contractAddress })">
                {{ $t('cosmwasm.btn_migrate') }}
            </label>

            <label for="wasm_update_admin" class="btn  #sav1 btn-xs text-xs mr-2"
                @click="dialog.open('wasm_update_admin', { contract: contractAddress })">
                {{ $t('cosmwasm.btn_update_admin') }}
            </label>

            <label for="wasm_clear_admin" class="btn  #sav1 btn-xs text-xs mr-2"
        ...
        <div class="#saved px-4 pt-3 pb-4 rounded mb-4 shadow">
            ...
            <table class="table">
                <thead style="color: white" class="backdrop-blur-md">
                    <tr>
                        <td> {{ $t('ibc.height') }}</td>
                        <td>{{ $t('ibc.txhash') }}</td>
                ...
                <tbody>
                    <tr v-for="resp in txs?.tx_responses">
                        <td>{{ resp.height }}</td>
                        <td>
                            <div class="text-xs truncate  #saved dark:invert">
                        ...
                        <td>
                            <div class="flex">
                                ...
                                <Icon v-if="resp.code === 0" icon="mdi-check" class="text-base-content text-lg" />
                    ...
                    <div>
                        ...
                        <div class="px-3">
                            ...
                            <div class="mt-4 mb-4 text-center">
                                <button class="btn  #sav1 px-4 text-white" @click="queryContract()">
