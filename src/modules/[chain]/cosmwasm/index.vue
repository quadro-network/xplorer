    ...
    <div class="#saved px-4 pt-3 pb-4 rounded mb-4 shadow">
        ...
        <div class="grid grid-flow-col auto-cols-max gap-4 overflow-hidden">
            <div class="join w-full border border-primary">
                ...
                <button class="join-item btn   #sav1" @click="myContracts()">{{ $t('cosmwasm.btn_query') }}</button>
            </div>
            <div>
        ...
        <div class="overflow-x-auto">
                    ...
                    <tr v-for="(v, index) in codes.code_infos" :key="index">
                        ...
                        <td>
                            <RouterLink :to="`/${props.chain}/cosmwasm/${v.code_id}/contracts`"
                                class="truncate max-w-[200px] block  #saved dark:invert" :title="v.data_hash">
                                {{ v.data_hash }}
                            </RouterLink>
                        </td>
                        <td>{{ v.creator }}</td>
            ...
            <div class="flex justify-between">
                ...
                <label for="wasm_store_code" class="btn  #sav1 my-5" @click="dialog.open('wasm_store_code', {})">{{ $t('cosmwasm.btn_up_sc') }}</label>
            </div>
        </div>
    </div>
</template>
