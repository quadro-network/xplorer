    ...
    <div class="#saved px-4 pt-3 pb-4 rounded shadow">
      <div class="flex flex-wrap gap-4  items-center">
        ...
        <div class="tabs tabs-boxed">
          <a class="tab" :class="{ 'tab-act': tab === 'registry' }" @click="tab = 'registry'">{{ $t('ibc.registry') }}</a>
          <a class="tab" :class="{ 'tab-act': tab === 'favorite' }" @click="tab = 'favorite'">{{ $t('module.favorite') }}</a>
        </div>
      </div>
      <div>
        ...
        <div v-show="tab === 'favorite'" class="flex flex-wrap gap-1 p-4 ">
          <div class="join border border-primary">
            ...
            <button class="join-item btn   #sav1" @click="ibcStore.showConnection()">{{ $t('ibc.btn_apply') }}</button>
          </div>
        </div>
      </div>
